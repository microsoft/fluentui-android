...
class AvatarViewDemoActivity : DemoActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        change_background.setOnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                background_view.setBackgroundColor(ContextCompat.getColor(baseContext, R.color.fluentui_gray_200))
            } else {
    ...
    private fun removeAndAddViews() {
        ...
        addLayout("Default Style", AvatarStyle.BASE, AvatarStyle.BASE)
        addLayout("Fallback(Default Style and accent style)", AvatarStyle.ACCENT, AvatarStyle.BASE, "+945249393",-1)
        addLayout("Fallback(Outline Style and Outlined Primary style)", AvatarStyle.OUTLINED, AvatarStyle.OUTLINED_PRIMARY, "+945249393",-1)
        addLayout("Group Style", AvatarStyle.GROUP, AvatarStyle.GROUP)
        addLayout("Group Style", AvatarStyle.OVERFLOW, AvatarStyle.OVERFLOW,"20",-1)
    }

    private fun addLayout(styleName: String, style1: AvatarStyle, style2: AvatarStyle,name: String = getString(R.string.persona_name_kat_larsson), drawable: Int = R.drawable.avatar_kat_larsson) {
        ...
        for (size in AvatarSize.values().reversedArray()) {
            ...
            tableRow.addView(TextView(baseContext).apply {
                text = size.name
    ...
    private fun createAvatarView(name: String, resourceId: Int, size: AvatarSize, style: AvatarStyle): AvatarView {
