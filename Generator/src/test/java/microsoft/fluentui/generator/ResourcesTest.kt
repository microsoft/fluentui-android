...
import com.msft.stardust.model.TextStyles
import com.msft.stardust.model.TypographyGenerator
...
class ResourcesTest {
    ...
    fun `TextStyles generation`() {
        ...
        Yaml().loadAs(document, StardustDocument::class.java).generate("it.path")
    ...
    fun `Global enums code generation`() {
        ...
        val outputfile = File("$global_flavorPath/java/com/microsoft/stardust/Enums.kt")
        Assert.assertTrue(outputfile.exists())
    ...
    fun `Animators generation`() {
        ...
        val animatorsEnumFile =
            File("$global_projectSourcePath/main/java/com/microsoft/stardust/Animators.kt")
    ...
    private fun testInterpolators(key: String, values: List<String>) {
        ...
        outputFile.delete()
        val sourceFile =
            File(global_projectSourcePath.plus("main/java/com/microsoft/stardust/Interpolators.kt"))
        if (values.size == 4 && sourceFile.exists()) {
            sourceFile.readText().let { content ->
                val interpolatorName =
                    "stardustInterpolator${key.substringAfterLast("_").capitalize()}"
                Assert.assertTrue(
                    content.contains(
