...
import microsoft.fluentui.generator.model.TextStyles
import microsoft.fluentui.generator.model.TypographyGenerator
...
class ResourcesTest {
    @Before
    fun setup() {
        global_projectRoot = _root_ide_package_.microsoft.fluentui.generator.testProjectPath
    ...
    @Test
    fun `Color selectors`() {
        testColorSelector("[rest: \"#000000\"]", false)
    ...
    fun `Global enums code generation`() {
        ...
        val outputfile = File("$global_flavorPath/java/microsoft/fluentui/generator/Enums.kt")
        Assert.assertTrue(outputfile.exists())
    ...
    fun `Animators generation`() {
        ...
        val animatorsEnumFile =
            File("$global_projectSourcePath/main/java/microsoft/fluentui/generator/Animators.kt")
    ...
    private fun testInterpolators(key: String, values: List<String>) {
        ...
        outputFile.delete()
        val sourceFile =
            File(global_projectSourcePath.plus("main/java/microsoft/fleuntui/generator/Interpolators.kt"))
    ...
    private fun testColorSelector(content: String, expected: Boolean) {
        ...
        val value = (doc["${AP_PREFIX}ButtonTokens"] as LinkedHashMap<String, Any>)["test"] as Any
        ...
        if (expected) {
            ...
            Assert.assertTrue(output.contains("android:state_pressed=\"true\""))
            Assert.assertTrue(output.contains("android:state_activated=\"true\""))
            Assert.assertTrue(output.contains("android:state_enabled=\"false\""))
            Assert.assertTrue(output.contains("<item android:color="))
        }
    }
