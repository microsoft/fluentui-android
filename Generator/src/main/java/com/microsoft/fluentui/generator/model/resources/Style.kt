...
fun generateStyles(sourcePath: String, name:String, styles: List<Style>) {
    generateStyles(sourcePath, name, styles, false)
}

fun generateStyles(sourcePath: String, name:String, styles: List<Style>, isVersion26Supported: Boolean) {
    StringBuilder().apply {
        ...
        if (isVersion26Supported) {
            toString().writeToResourceFile(
                    sourcePath.plus("res/values-v26/${name}_styles.autogenerated.xml"),
                    "${styles.count()} styles"
            )
        }
        else{
            toString().writeToResourceFile(
                    sourcePath.plus("res/values/${name}_styles.autogenerated.xml"),
                    "${styles.count()} styles"
            )
        }
    }
