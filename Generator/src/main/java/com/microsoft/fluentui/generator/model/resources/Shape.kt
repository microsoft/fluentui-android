...
fun generateShape(sourcePath: String, name: String, values: Map<ShapeKeys, String>) {

    StringBuilder().apply {
        append(AUTOGENERATED_XML_HEADER)
        append(intGenerateShape(includeNamespace = true, includeItemTag = false, values = values))
        toString().writeToResourceFile(sourcePath.plus("res/drawable/$name.xml"), "Shape")
    }

    StringBuilder().apply {
        append(AUTOGENERATED_XML_HEADER)
        append("<ripple xmlns:android=\"http://schemas.android.com/apk/res/android\"")
        if(values[ShapeKeys.RIPPLE_COLOR] != null)
            append(" android:color=\"${values[ShapeKeys.RIPPLE_COLOR]}\">\n")
        else
            append(" android:color=\"@color/fluentui_transparent\">\n")
        append(intGenerateShape(includeNamespace = false, includeItemTag = true, values = values))
        append("</ripple>")

        toString().writeToResourceFile(sourcePath.plus("res/drawable-v21/$name.xml"), "Shape")
    }
}
    ...
    return StringBuilder().apply {
        ...
        values.forEach {
            when (it.key) {
                ...
                ShapeKeys.BACKGROUND_COLOR -> append("\t\t<solid android:color=\"@color/${it.value}\"/>\n")
                ShapeKeys.STROKE_COLOR -> {
                    append("\t\t<stroke ")
                    values[ShapeKeys.STROKE_WIDTH]?.let { width -> append("android:width=\"@dimen/$width\" ") }
                    append("android:color=\"@color/${it.value}\"/>\n")
                }
                else -> Unit
            }
        }
