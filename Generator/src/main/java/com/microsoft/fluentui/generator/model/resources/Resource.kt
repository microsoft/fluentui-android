...
import com.microsoft.fluentui.generator.model.ENUM_PATTERN
import com.microsoft.fluentui.generator.model.global_getResourceType
import java.util.*
            ...
            if (it.type == ResourceType.TYPED_ARRAY) {
                when {
                    ...
                    it.resourceList  == null || it.resourceList.isEmpty() -> printError("Found typed array resource with no vakues!")
                        ...
                        append("\t<array name=\"${ if (prependNameToResourceName) "${name}_" else "" }${ it.name }\">\n")
...
fun generateTokenSystemFile(
    flavorPath: String,
    importPath: String,
    stylesheetName: String,
    viewName: String,
    parameters: LinkedHashMap<String, Any>?,
    drawables: List<Resource>?
){
    StringBuilder().apply {
        val stringBuilder = this
        val styleTokenValue = TokenType()
        val otherTokenValues = ArrayList<TokenType>()
        val tokenVars = ArrayList<TokenVar>()
        append(AUTOGENERATED_CODE_HEADER)
        append("package com.microsoft.fluentui.generator.resourceProxies")
        append(NEW_LINE)
        append(createGenericImports(importPath))
        append(NEW_LINE)

        append("open class $stylesheetName" + "System(val context: Context, val resources: Resources)")
        ...
        parameters?.apply {
            forEach { entry ->
                ...
                when {
                    Regex(ENUM_PATTERN).matches(value) -> {
                        var firstValue:String = ""
                        var values:ArrayList<String> = ArrayList()
                        Regex(STRING_PATTERN).findAll(value).single().value.split(',').let {
                        ...
                        if(entry.key.toUpperCase().contains(ResourceType.STYLE.toString())){
                            styleTokenValue.name = entry.key
                            styleTokenValue.valueList = values
                        }
                        else {
                            otherTokenValues.add(TokenType(entry.key,values))
                        }
                        generateExposedValues(entry.key,firstValue,stringBuilder)
                    }
                    else -> {
                        val resourceList = mutableListOf<Resource>()
                        parseResourceCodeWrapper(resourceList, entry.key, entry.value)
                        if(resourceList.isNotEmpty()) {
                            val resource = resourceList[0]
                            when(resource.type){
                                ResourceType.COLOR->{
                                    if(resourceList.size == 1) {
                                        tokenVars.add(TokenVar(entry.key,styleTokenValue, ResourceType.COLOR,false))
                                    }
                                    else {
                                        tokenVars.add(TokenVar(entry.key,styleTokenValue, ResourceType.COLOR, true))
                                    }
                                    append("var ${entry.key}: ")
                                    append("Int = 0\n")
                                }
                                ResourceType.DIMENSION -> {
                                    setupTokens("Float","f",resource, otherTokenValues, tokenVars, entry.key, resourceList, stringBuilder)
                                }
                                ResourceType.FONT_STYLE,
                                    ResourceType.STYLE,
                                ResourceType.TYPED_ARRAY ->{
                                    setupTokens("Int","",resource, otherTokenValues, tokenVars, entry.key, resourceList, stringBuilder)
                                }
                                else -> {
                                    // Do nothing, right now not handled
                                }
                            }
                        }
                    }
        ...
        if(drawables != null && drawables.size>0) {
            setupTokens("Drawable?",null, drawables[0], otherTokenValues, tokenVars, "background",
                drawables as MutableList<Resource>, this )
        }
        append("\n")
        append("private fun updateProxy(){\n")
        addTokenVariableBlock("val appearProxy ", styleTokenValue.name, styleTokenValue.valueList,"","${stylesheetName}Proxy(context,resources)", {it.capitalFirst()},stringBuilder)
        tokenVars.forEach { entry ->
            when(entry.resourceType) {
                ResourceType.COLOR -> {
                    if (entry.isLowerCase) {
                        addTokenVariableBlock("${entry.name} ", entry.tokenType?.name, entry.tokenType?.valueList, "ContextCompat.getColor(context, R.color.", "${stylesheetName.toLowerCase()}view_${entry.name.toLowerCase()})", {it.toLowerCase()}, stringBuilder)

                    } else {
                        addTokenVariableBlock("${entry.name} ", entry.tokenType?.name, entry.tokenType?.valueList, "ContextCompat.getColor(context, R.color.", "${stylesheetName.toLowerCase()}view_${entry.name})", {it.toLowerCase()}, stringBuilder)
                    }
                }
                else -> {
                    addTokenVariableBlock("${entry.name} ", entry.tokenType?.name, entry.tokenType?.valueList, "appearProxy.${entry.name}", "",{"_$it"} , stringBuilder)
                }
            }
        }
        append("}\n")
...
fun setupTokens(variableType: String,variableVal: String?, resource: Resource, otherTokenValues: ArrayList<TokenType>, tokenVars: ArrayList<TokenVar>, entryName: String, resourceList: MutableList<Resource>, stringBuilder: StringBuilder) {
    var isOfTokenType = false
    for( token in otherTokenValues) {
        val tokenValues = token.valueList
        if (tokenValues.isNullOrEmpty()){
            continue
        }
        for (tokenTypeValue in tokenValues) {
            if (resource.name.contains(tokenTypeValue, true)) {
                tokenVars.add(TokenVar(entryName,token, resource.type, false))
                isOfTokenType = true
                break
            }
        }
    ...
    stringBuilder.apply {
        if (isOfTokenType) {
            append("var ${entryName}: ")
            if (variableVal == null) {
                append("$variableType = null\n")
            }
            else {
                append("$variableType = 0${variableVal}\n")
            }
        }
        else {
            for( resource in resourceList) {
                tokenVars.add(TokenVar(resource.name,null, resource.type, false))
                append("var ${resource.name}: ")
                if (variableVal == null) {
                    append("$variableType = null\n")
                }
                else {
...
fun addTokenVariableBlock(variableDeclarationName: String,tokenTypeName: String?, tokenTypeList: ArrayList<String>?,preVariableValue:String,postVariableValue:String,stringFun: (String)->String, stringBuilder: StringBuilder) {
    stringBuilder.apply {
        append(TAB)
        append(variableDeclarationName).append(" = ")
        if(tokenTypeList.isNullOrEmpty()){
            append("$preVariableValue$postVariableValue\n")
        }
        else {
            append("when(${tokenTypeName?.camelize()}) {\n")
            tokenTypeList.forEach {
                append(TAB)
                append(TAB)
                if (it.equals("base",true)){
                    append("$tokenTypeName.${it.toJavaEnumValueName()}-> $preVariableValue$postVariableValue\n")
                }
                else {
                    append("$tokenTypeName.${it.toJavaEnumValueName()}-> $preVariableValue${stringFun(it)}$postVariableValue\n")
                }
            }
            append(TAB)
            append("}\n")
...
fun generateExposedValues(entry: String, type: String, stringBuilder: StringBuilder) {
    stringBuilder.apply {
        append("var ").append(entry.camelize()).append(":").append(" ").append(entry)
                .append(" = ")
                .append(entry).append(".")
                .append(type.split(",")[0].toJavaEnumValueName())
                .append("\n")
                .append(TAB)
                .append("set(value) {")
                .append("\n")
                .append(TAB).append(TAB)
                .append("if (field == value) return")
                .append("\n")
                .append(TAB).append(TAB)
                .append("field = value")
                .append("\n")
        ...
        StringBuilder().apply {
            ...
            values.forEach {
                when {
                    ...
                    it.type == ResourceType.FONT_STYLE -> append(
                        createFontStyleVal(
                                viewName,
                                parentParameters,
                                it
                    ...
                    it.type == ResourceType.TYPED_ARRAY -> append(
                            createArrayVal(
                                    viewName,
                                    parentParameters,
                                    it
                    ...
                    it.type == ResourceType.DRAWABLE -> append(
                        createDrawableVal(
                            viewName,
                            parentParameters,
                            it
...
fun createFontStyleVal(
    ...
    return prefix + resource.name + ": Int = R.style.${name.toLowerCase(Locale.ENGLISH)}_typography_" + resource.name + "\n"
}
...
fun createArrayVal(
    ...
    return prefix + resource.name + ": Int = R.array." + "${name.toLowerCase(
            Locale.ENGLISH
    )}_" + resource.name + "\n"
}

fun createDrawableVal(
    name: String,
    parentParameters: LinkedHashMap<String, Any>?,
    resource: Resource
): String {
    ...
    return prefix + resource.name +": Drawable? = ContextCompat.getDrawable(context, ${resource.value})\n"
}
...
fun createEnumVal(
    ...
    val enumClassName = "com.microsoft.fluentui.generator" + resource.value.enumClassName()
    val prefix = parentParameters?.let {
        if (it.containsKey(resource.name.substringBefore("_"))) OVERRIDE_VAL_PREFIX else OPEN_VAL_PREFIX
    } ?: OPEN_VAL_PREFIX
...
fun createGenericImports(importPath: String): String {
    return "\nimport android.content.Context\n" +
            ...
            "import com.microsoft.fluentui.${importPath}R\n"
}
        ...
        when (values) {
            is ArrayList<*> -> {
                if (values[0] is String) {
                    ...
                    val arrayResources: MutableList<Resource> = mutableListOf()
                    values.forEachIndexed { i, v ->
                        parseResourceCodeWrapper(arrayResources, resKey, v)
                    }
                    ...
                    values.forEach { v -> parseResourceCodeWrapper(resourceList, resKey, v) }
                }
            }
            is LinkedHashMap<*, *> -> {
                // check if the key is an compile-time expression modifier
                ...
                when {
                    ...
                    Regex(CompositePrimitiveType.FLUENTUICOLOR.pattern).matches(values) -> {
                        ...
                        foundValues.forEachIndexed { _, result ->
                            ...
                            // Capture any of light or dark color variants in a single pass.
                            colorMap["light"]?.let {
                                resourceList.add(
                                        Resource(
                                                resKey,
