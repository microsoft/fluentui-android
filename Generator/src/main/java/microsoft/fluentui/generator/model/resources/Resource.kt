        ...
        with(stringBuilder) {
            if (it.type == ResourceType.TYPED_ARRAY) {
                when {
                    ...
                    it.resourceList.isNullOrEmpty() -> printError("Found typed array resource with no vakues!")
                    else -> {
...
fun generateComponentProxyFile(
    ...
    if (values.isEmpty()) {
        ...
        StringBuilder().apply {
            ...
            append("package com.microsoft.stardust.resourceProxies")
            append(NEW_LINE)
...
fun createEnumVal(
    ...
    val enumClassName = "com.microsoft.stardust." + resource.value.enumClassName()
    val prefix = parentParameters?.let {
        if (it.containsKey(resource.name.substringBefore("_"))) OVERRIDE_VAL_PREFIX else OPEN_VAL_PREFIX
    } ?: OPEN_VAL_PREFIX
...
fun createGenericImports(): String {
    return "\nimport android.content.Context\n" +
            ...
            "import com.microsoft.stardust.R\n"
}
